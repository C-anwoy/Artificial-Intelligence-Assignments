...
class Model():
    ...
    def backward(self, y_pred, y_actual, features):
        ...
        n = y_actual.shape[0]  # number of training examples
    
        alpha=self.lr
        
        (self.W)=(self.W)-(alpha*(1/n)*((np.transpose(features))@(y_pred-y_actual)))
    ...
    def loss(self, y_pred, y_actual):
        ...
        loss=(1/(2*n))*(((y_pred-y_actual)**2).sum())
...
def get_features(data, data_type):
    ...
    if data_type == 'linear':
        ...
        features=np.append(data,np.ones((data.shape[0],1)),axis=1)
        ...
        features=np.append(data**3,data**2,axis=1)
        features=np.append(features,data,axis=1)
        features=np.append(features,np.ones((data.shape[0],1)),axis=1)
        ## YOUR CODE ENDS HERE ##        
        ...
        ## YOUR CODE STARTS HERE ##
        features=np.append(data,np.sin(data),axis=1)
        features=np.append(features,np.cos(data),axis=1)
        features=np.append(features,np.ones((data.shape[0],1)),axis=1)
